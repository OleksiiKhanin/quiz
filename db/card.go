...
func (c *cardTransaction) InsertCard(ctx context.Context, card *dto.Card) (int64, error) {
	var id int64
	query := fmt.Sprintf("INSERT INTO %s (value, description, type, lang, image_hash) VALUES ($1,$2,$3,$4,$5) RETURNING id", cardTableName)
...
func (c *cardTransaction) GetCard(ctx context.Context, id int64) (dto.Card, error) {
	query := fmt.Sprintf("SELECT id, value, description, type, lang, image_hash, added_at FROM %s WHERE id=%d", cardTableName, id)
	var card dto.Card
	err := c.tx.QueryRowxContext(ctx, query).
		Scan(&card.ID, &card.Value, &card.Description, &card.Type, &card.Lang, &card.ImageHash, &card.AddedAt)
	if err != nil {
		c.tx.Rollback()
		return card, err
	}
